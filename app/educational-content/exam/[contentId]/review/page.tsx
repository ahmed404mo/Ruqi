...
export default function ExamReviewPage({ params }: { params: Promise<{ contentId: string }> }) {
  ...
  return (
    <div
      className="w-full min-h-screen bg-background flex flex-col items-center py-12 md:py-20 px-4 md:px-8 font-cairo"
      dir="rtl"
    >
      <main className="flex flex-col items-start gap-8 w-full max-w-[880px] flex-1">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full bg-surface border border-border shadow-sm rounded-card p-6">
          <div className="flex flex-col gap-1">
            <h1 className="font-black text-[20px] md:text-[24px] text-text-main">مراجعة إجابات: {content.title}</h1>
            <span className="font-medium text-[14px] text-text-muted">
              راجع أخطائك وتعلم منها لضمان التفوق في الاختبارات القادمة.
            </span>
          </div>
          <Link
            href={`/educational-content/exam/${contentId}/result`}
        ...
        <div className="flex flex-col gap-8 w-full">
          {questions.map((question, qIndex) => {
            ...
            return (
              <div
                key={qIndex}
                className="w-full bg-surface border border-border shadow-sm rounded-card p-6 md:p-8 flex flex-col gap-6"
              >
                <div className="flex items-center gap-3 border-b border-border pb-4">
                  <span className="font-extrabold text-[16px] text-text-main flex-1">السؤال {qIndex + 1}:</span>
                  {isCorrectAnswered ? (
                  ...
                  {question.options.map((option, optIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {Icon && <div className="shrink-0 mr-3">{Icon}</div>}
                      </div>
                    );
                  })}
                </div>
